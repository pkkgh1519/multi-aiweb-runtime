...
function Write-McpJson {
    ...
    $payload | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $McpJson -Encoding UTF8
}
...
function Update-MarketplaceManifest {
    ...
    if (Test-Path -LiteralPath $MarketplaceConfig) {
        $existing = Get-Content -LiteralPath $MarketplaceConfig -Raw | ConvertFrom-Json
    ...
    $payload | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $MarketplaceConfig -Encoding UTF8
    return $resolvedName
}
...
function Write-InstallManifest([string]$ResolvedMarketplaceName) {
    ...
    $payload | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $InstallManifest -Encoding UTF8
}
