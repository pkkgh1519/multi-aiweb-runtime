...
# Multi-model API run
npx -y @steipete/oracle -p "Cross-check the data layer assumptions" --models gpt-5.1-pro,gemini-3.1-pro --file "src/**/*.ts"
...
npx -y @steipete/oracle doctor --providers --models gpt-5.5-pro,gemini-3.1-pro,claude-4.6-sonnet

# Multi-model advisory panel with recoverable partial success
npx -y @steipete/oracle --models gpt-5.5-pro,gemini-3.1-pro,claude-4.6-sonnet \
...
- API mode expects API keys in your environment: `OPENAI_API_KEY` (GPT-5.x), `GEMINI_API_KEY` (Gemini provider routes), `ANTHROPIC_API_KEY` (Claude Sonnet 4.6 / Opus 4.1).
- Gemini browser mode uses Chrome cookies instead of an API key—just be logged into `gemini.google.com` in Chrome (no Python/venv required). Supported Web picker models are `gemini-3.1-pro`, `gemini-3.5-flash`, and `gemini-3.1-flash-lite`.
- Gemini browser mode fails closed if the requested Web model is unavailable or appears to fall back to a different effective model; it does not record success under the requested model in that case.
...
```bash
oracle doctor --providers --models gpt-5.4,claude-4.6-sonnet,gemini-3.1-pro
oracle --preflight --models gpt-5.4,gemini-3.1-pro
...
| `-m, --model <name>`                                                           | Built-ins (`gpt-5.5-pro` default, `gpt-5.5`, `gpt-5.4-pro`, `gpt-5.4`, `gpt-5.1-pro`, `gpt-5-pro`, `gpt-5.1`, `gpt-5.1-codex`, `gpt-5.2`, `gpt-5.2-instant`, `gpt-5.2-pro`, Gemini Web models `gemini-3.1-pro` / `gemini-3.5-flash` / `gemini-3.1-flash-lite`, `claude-4.6-sonnet`, `claude-4.1-opus`) plus any OpenRouter id (e.g., `minimax/minimax-m2`, `openai/gpt-4o-mini`). |
