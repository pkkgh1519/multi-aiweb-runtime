...
describe("resolveRunOptionsFromConfig", () => {
  ...
  it("preserves gemini-3.5-flash in auto-selected browser mode", () => {
    const { runOptions, resolvedEngine, engineCoercedToApi } = resolveRunOptionsFromConfig({
      prompt: basePrompt,
    ...
    expect(resolvedEngine).toBe("browser");
    expect(engineCoercedToApi).toBe(false);
    expect(runOptions.model).toBe("gemini-3.5-flash");
    expect(runOptions.effectiveModelId).toBe("gemini-3.5-flash");
  });

  it("preserves explicit browser engine for Gemini 3.5 Flash labels", () => {
  ...
  it("preserves Gemini 3.1 Flash-Lite in explicit browser mode", () => {
    const { resolvedEngine, runOptions } = resolveRunOptionsFromConfig({
      prompt: basePrompt,
    ...
    expect(resolvedEngine).toBe("browser");
    expect(runOptions.model).toBe("gemini-3.1-flash-lite");
    expect(runOptions.effectiveModelId).toBe("gemini-3.1-flash-lite");
  });

  it("rejects unsupported explicit Gemini browser models before executor handoff", () => {
    expect(() =>
  ...
  it("keeps Gemini 3.1 Pro in browser mode when auto-detected", () => {
    const { runOptions, resolvedEngine, engineCoercedToApi } = resolveRunOptionsFromConfig({
      prompt: basePrompt,
      model: "gemini-3.1-pro",
      env: {},
    ...
    expect(engineCoercedToApi).toBe(false);
    expect(runOptions.model).toBe("gemini-3.1-pro");
    expect(runOptions.effectiveModelId).toBe("gemini-3.1-pro");
  });

  it("rejects retired Gemini browser models before executor handoff", () => {
    expect(() =>
      resolveRunOptionsFromConfig({
        prompt: basePrompt,
        model: "gemini-3-pro",
  ...
  it("normalizes shorthand multi-model entries", () => {
    ...
    expect(runOptions.model).toBe("gpt-5.1");
    expect(runOptions.models).toEqual(["gpt-5.1", "gemini-3.1-pro", "claude-4.6-sonnet"]);
  });
