...
describe("provider doctor CLI", () => {
  ...
  test(
    ...
    async () => {
      ...
      const { stdout } = await execFileAsync(
        process.execPath,
        ["--import", "tsx", CLI_ENTRY, "--preflight", "--models", "gpt-5.4,gemini-3.1-pro"],
        { env },
      );

      expect(stdout).toContain("Provider preflight");
      expect(stdout).toContain("gpt-5.4: ok");
      expect(stdout).toContain("gemini-3.1-pro: ok");
  ...
  test(
    ...
    async () => {
      ...
      const { stdout } = await execFileAsync(
        process.execPath,
        ["--import", "tsx", CLI_ENTRY, "--route", "--models", "gpt-5.4,gemini-3.1-pro"],
        { env },
      );
