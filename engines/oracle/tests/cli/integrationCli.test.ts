...
`,
      ...
      if (process.platform === "win32" || exit.signal === "SIGINT") {
  ...
  test(
    "preserves Gemini 3.5 Flash at the browser dry-run CLI boundary",
    async () => {
      const oracleHome = await mkdtemp(path.join(os.tmpdir(), "oracle-provider-gemini-35-"));
      const env: NodeJS.ProcessEnv = {
        ...process.env,
        // biome-ignore lint/style/useNamingConvention: env var name
        DOTENV_CONFIG_PATH: "/tmp/nonexistent-oracle-env",
      ...
      expect(stdout).toContain("browser mode (gemini-3.5-flash)");
      expect(stdout).not.toContain("browser mode (gemini-3.1-pro)");
  ...
  test(
    "rejects unsupported explicit Gemini browser models at the dry-run CLI boundary",
    async () => {
      const oracleHome = await mkdtemp(path.join(os.tmpdir(), "oracle-provider-gemini-fake-"));
      const env: NodeJS.ProcessEnv = {
        ...process.env,
        // biome-ignore lint/style/useNamingConvention: env var name
        DOTENV_CONFIG_PATH: "/tmp/nonexistent-oracle-env",
      ...
      expect(result.code).toBe(1);
      expect(`${result.stdout}\n${result.stderr}`).toContain("Unsupported Gemini model");
      expect(result.stdout).not.toContain("browser mode (gemini-3.1-pro)");
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        expect(stderr).toMatch(/gemini-3.1-pro/i);
      }
    ...
    async () => {
      ...
      await execFileAsync(
        ...
        [
          ...
          "--models",
          "gpt-5.1,gemini-3.1-pro,claude-4.6-sonnet",
        ],
        { env },
      );
      ...
      expect(selectedModels).toEqual(
        expect.arrayContaining(["gpt-5.1", "gemini-3.1-pro", "claude-4.6-sonnet"]),
      );
      expect(metadata.status).toBe("completed");
  ...
  test(
    ...
    async () => {
      ...
      const env = {
        ...
        ORACLE_NO_DETACH: "1",
        // biome-ignore lint/style/useNamingConvention: env var name
        ORACLE_TEST_FAIL_MODEL: "gemini-3.1-pro",
      };
      ...
      expect(manifest.models[1].logPath).toContain("gemini-3.1-pro.log");
      ...
      expect(result.stdout).toContain("gemini-3.1-pro");
