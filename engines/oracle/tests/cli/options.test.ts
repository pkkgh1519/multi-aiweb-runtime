...
describe("resolveApiModel", () => {
  ...
  test("rejects retired Gemini deep-think aliases", () => {
    expect(() => resolveApiModel("gemini-3-deep-think")).toThrow("Unsupported Gemini model");
    expect(() => resolveApiModel("Gemini Deep Think")).toThrow("Unsupported Gemini model");
  });

  test("rejects unsupported explicit Gemini model strings", () => {
    expect(() => resolveApiModel("gemini-9-fake")).toThrow("Unsupported Gemini model");
    expect(() => resolveApiModel("gemini-3.5-pro")).toThrow("Unsupported Gemini model");
  });
...
describe("inferModelFromLabel", () => {
  ...
  test("preserves Gemini 3.5 Flash labels for browser mode", () => {
    expect(inferModelFromLabel("Gemini 3.5 Flash")).toBe("gemini-3.5-flash");
    expect(inferModelFromLabel("gemini-3.5-flash")).toBe("gemini-3.5-flash");
  });

  test("preserves current Gemini browser labels", () => {
    expect(inferModelFromLabel("Gemini 3.1 Pro")).toBe("gemini-3.1-pro");
    expect(inferModelFromLabel("Gemini 3.1 Flash-Lite")).toBe("gemini-3.1-flash-lite");
  });

  test("rejects unsupported explicit Gemini labels", () => {
    expect(() => inferModelFromLabel("gemini-9-fake")).toThrow("Unsupported Gemini model");
    expect(() => inferModelFromLabel("Gemini 3.5 Pro")).toThrow("Unsupported Gemini model");
  });
