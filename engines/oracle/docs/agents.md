...
- Oracle bundles a prompt plus the right files so a Pro model (GPT-5.5 Pro,
  Gemini 3.1 Pro, Claude Opus) can answer with real repo context. Use when stuck,
...
- **Plan → Oracle → execute.** Draft the plan, ask Claude Opus or Gemini 3.1 Pro to challenge it, then implement.
