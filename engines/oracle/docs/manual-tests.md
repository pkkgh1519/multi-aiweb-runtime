...
1. Kick off an API multi-run:
   `pnpm run oracle -- --models "gpt-5.1-pro,gemini-3.1-pro" --prompt "Compare the moon & sun."`
   - Expect stdout to print sequential sections, one per model (`[gpt-5.1-pro] …` followed by `[gemini-3.1-pro] …`). No interleaved tokens.
...
3. Inspect detailed logs: `oracle session <id>`
   ...
   - When prompted, pick `View gemini-3.1-pro log` and confirm only that model’s stream renders. Refresh should keep completed models intact even if others still run.
4. Model filter path: `oracle session <id> --model gemini-3.1-pro`
...
3. Multi-model spot-check: `oracle --models "gpt-5.1-pro,gemini-3.1-pro" --prompt "two files" --write-output /tmp/out.md --wait`
   - Confirm `/tmp/out.gpt-5.1-pro.md` and `/tmp/out.gemini-3.1-pro.md` exist with distinct content.
