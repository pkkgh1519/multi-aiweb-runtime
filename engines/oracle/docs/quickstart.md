...
```bash
oracle doctor --providers --models gpt-5.5-pro,gemini-3.1-pro,claude-4.6-sonnet
```
...
```
- Oracle bundles a prompt plus the right files so a Pro model (GPT-5.5 Pro, Gemini 3.1 Pro, Claude Opus) can answer. Use when stuck, debugging, or reviewing.
