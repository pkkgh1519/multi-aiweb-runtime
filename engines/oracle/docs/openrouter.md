...
- For built-in API-capable models (`gpt-5.1-pro`, `claude-4.6-sonnet`, Gemini provider models), Oracle falls back to OpenRouter automatically when the provider key is missing but `OPENROUTER_API_KEY` is set. It keeps using first-party endpoints when their keys are present.
