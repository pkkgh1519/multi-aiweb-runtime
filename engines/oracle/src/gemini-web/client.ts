...
export type GeminiWebThinkingLevel = "standard" | "extended";
...
const MODEL_HEADERS: Record<GeminiWebModelId, Partial<Record<GeminiWebThinkingLevel, string>>> = {
...
function resolveGeminiWebModelHeader(
  model: GeminiWebModelId,
  thinkingLevel: GeminiWebThinkingLevel | undefined,
): string {
  const modelHeaders = MODEL_HEADERS[model];
  if (!modelHeaders) {
    throw new Error(
      `Unsupported Gemini web model "${model}". Use gemini-3.1-pro, gemini-3.5-flash, or gemini-3.1-flash-lite.`,
    );
  }
  ...
  if (!header) {
    throw new Error(
      `Unsupported Gemini Web thinking level "${String(level)}". Current Gemini Web picker exposes Standard and Extended only.`,
    );
  }
  return header;
}
...
export async function runGeminiWebWithFallback(
  ...
  if (isGeminiModelUnavailable(attempt.errorCode)) {
    throw new Error(
      `Requested Gemini web model "${input.model}" is unavailable in Gemini Web. Refusing to fall back to another model.`,
    );
  }
