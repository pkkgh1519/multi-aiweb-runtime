...
function resolveGeminiWebModel(
  ...
  switch (normalized) {
    ...
    default:
      throw new Error(
        `Unsupported Gemini web model "${desired}". Use gemini-3.1-pro, gemini-3.5-flash, or gemini-3.1-flash-lite.`,
      );
  }
}
...
function resolveGeminiThinkingLevel(
  ...
  throw new Error(
    `Unsupported Gemini Web thinking level "${requested}". Current Gemini Web picker exposes Standard and Extended only.`,
  );
}

function assertGeminiThinkingLevelSupported(
...
function assertEffectiveGeminiWebModel(
  effectiveModel: GeminiWebModelId | undefined,
  requestedModel: GeminiWebModelId,
): void {
  if (effectiveModel !== requestedModel) {
    throw new Error(
      `Blocked Gemini web fallback: effective Gemini web model "${effectiveModel ?? "unknown"}" did not match requested "${requestedModel}".`,
    );
  }
}
