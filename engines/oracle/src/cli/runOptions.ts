...
export function resolveRunOptionsFromConfig({
  ...
  const isGeminiBrowserModel = (m: string) =>
    m === "gemini-3.1-pro" || m === "gemini-3.5-flash" || m === "gemini-3.1-flash-lite";
  const isBrowserCompatible = (m: string) => m.startsWith("gpt-") || isGeminiBrowserModel(m);
  ...
  const engineCoercedToApi =
    engineWasBrowser && (isCodex || isClaude || isGrok || azureAutoApi);
