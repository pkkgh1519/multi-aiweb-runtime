...
export async function buildBrowserConfig(
  ...
  validateGeminiThinkingTime(options.model, options.browserThinkingTime);
...
function validateGeminiThinkingTime(model: ModelName, thinkingTime: ThinkingTimeLevel | undefined): void {
  const normalizedModel = model.toLowerCase().replace(/[_\s]+/g, "-");
  if (!normalizedModel.startsWith("gemini") || !thinkingTime) {
    return;
  }
  if (thinkingTime !== "standard" && thinkingTime !== "extended") {
    throw new Error(
      "Gemini Web thinking level must be standard or extended; the current picker does not expose light or heavy.",
    );
  }
  const isGeminiPro =
    ...
    normalizedModel === "gemini-3-1-pro";
  if (thinkingTime === "extended" && !isGeminiPro) {
    throw new Error(
      `Gemini Web thinking level "extended" is only supported for gemini-3.1-pro; ${model} supports standard only.`,
    );
  }
}
...
export function mapModelToBrowserLabel(model: ModelName): string {
  ...
  if (normalized.toLowerCase().startsWith("gemini")) {
    throw new Error(
      `Unsupported Gemini browser model "${model}". Use gemini-3.1-pro, gemini-3.5-flash, or gemini-3.1-flash-lite.`,
    );
  }
