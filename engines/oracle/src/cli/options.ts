...
const GEMINI_BROWSER_MODEL_ALIASES: Record<string, ModelName> = {
...
function normalizeGeminiAliasInput(normalized: string): string {
  return normalized.replace(/[ _]+/g, "-").replace(/--+/g, "-");
}

function unsupportedGeminiModelError(normalized: string): InvalidArgumentError {
  return new InvalidArgumentError(
    `Unsupported Gemini model "${normalized}". Use gemini-3.1-pro, gemini-3.5-flash, or gemini-3.1-flash-lite for Gemini browser mode.`,
  );
}
