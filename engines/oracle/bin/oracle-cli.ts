...
program
  ...
  .option(
    "-m, --model <model>",
    'Model to target (gpt-5.5-pro default). Also gpt-5.5, gpt-5.4-pro, gpt-5.4, gpt-5.1-pro, gpt-5-pro, gpt-5.1, gpt-5.1-codex API-only, gpt-5.2, gpt-5.2-instant, gpt-5.2-pro, Gemini Web models gemini-3.1-pro / gemini-3.5-flash / gemini-3.1-flash-lite, claude-4.6-sonnet, claude-4.1-opus, or ChatGPT labels like "5.5 Pro" / "5.2 Thinking" for browser runs).',
    normalizeModelOption,
  )
  .addOption(
    new Option(
      "--models <models>",
      'Comma-separated API/browser model list to query in parallel (e.g., "gpt-5.5-pro,gemini-3.1-pro").',
    )
...
async function runRootCommand(options: CliOptions): Promise<void> {
  ...
  const isGeminiBrowserModel = (model: string) =>
    model === "gemini-3.1-pro" || model === "gemini-3.5-flash" || model === "gemini-3.1-flash-lite";
  const isBrowserCompatible = (model: string) =>
    model.startsWith("gpt-") || isGeminiBrowserModel(model);
