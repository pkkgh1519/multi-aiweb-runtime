...
function validateGeminiThinkingTime(model, thinkingTime) {
    ...
    if (thinkingTime !== "standard" && thinkingTime !== "extended") {
        throw new Error("Gemini Web thinking level must be standard or extended; the current picker does not expose light or heavy.");
    }
    const isGeminiPro = normalizedModel === "gemini" ||
...
export function mapModelToBrowserLabel(model) {
    ...
    if (normalized.toLowerCase().startsWith("gemini")) {
        throw new Error(`Unsupported Gemini browser model "${model}". Use gemini-3.1-pro, gemini-3.5-flash, or gemini-3.1-flash-lite.`);
    }
